...
| constrained-random (`@constraint`, `randomize()`, `@rand_layer`, `layered_randomize()`) | covered | covered (generated `constraint {}` and `layered_randomize()`) | excluded (no C++ `randomize()`) | deferred | Python SAT/UNSAT/`state_xz` and layered status; SV simulator sampling need not match Python bit-for-bit |
